...
class ChatServer:
    ...
    async def _remove_user(self, username: str):
        ...
        try:
            ...
            logging.exception('Ошибка при закрытии клиентского писателя, игнорируется.', exc_info=error)
    ...
    async def _listen_for_messages(self, username: str, reader: StreamReader):
        try:
            ...
            logging.exception('Ошибка при чтении данных от клиента.', exc_info=error)
    ...
    async def _notify_all(self, message: str):
        ...
        for username, writer in self._username_to_writer.items():
            try:
                ...
                logging.exception('Ошибка при записи данных клиенту.', exc_info=error)
